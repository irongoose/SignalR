...
import { CancelInvocation, CompletionMessage, HubMessage, IHubProtocol, InvocationMessage, MessageType, NegotiationMessage, ProtocolType, ResultMessage, StreamInvocationMessage } from "./IHubProtocol";
...
export class HubConnection {
        ...
        for (const message of messages) {
            switch (message.type) {
                case MessageType.Invocation:
                    this.invokeClientMethod(message as InvocationMessage);
                    break;
                case MessageType.StreamItem:
                case MessageType.Completion:
                    const callback = this.callbacks.get((message as any).invocationId);
                    if (callback != null) {
                        if (message.type === MessageType.Completion) {
                            this.callbacks.delete((message as any).invocationId);
                        }
                        callback(message);
                    }
                    break;
    ...
    public stream<T>(methodName: string, ...args: any[]): Observable<T> {
        ...
        const subject = new Subject<T>(() => {
            const cancelInvocation: CancelInvocation = this.createCancelInvocation(invocationDescriptor.invocationId);
